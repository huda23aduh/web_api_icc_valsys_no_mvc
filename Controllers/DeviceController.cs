...
namespace web_api_icc_valsys_no_mvc.Controllers
{
    public class DeviceController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/device/addNewSerialNumber")]
        public HttpResponseMessage addNewSerialNumber([FromBody] New_sn_item req)
        {
            int is_sukses = 0;
            BuildList aa = new BuildList();

            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader authHeader = var_auth.getAuthHeader(req.username_ad, req.password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);
            
            var devServ = AsmRepository.AllServices.GetDevicesService(authHeader);

            var stock_receive_detail = new StockReceiveDetails();

            stock_receive_detail.FromStockHanderId = req.stock_receive_Detail_id; //32732
            stock_receive_detail.ToStockHanderId = req.stock_receive_Detail_id; //32732
            stock_receive_detail.Reason = 71197;

            var uu = devServ.CreateStockReceiveDetails(stock_receive_detail);


            var the_build_list = new BuildList();

            the_build_list.Reason = 71197;
            the_build_list.ModelId = req.model_id; //22
            the_build_list.StockReceiveDetailsId = uu.Id;
            the_build_list.StockHandlerId = req.stock_handler_id ; //33322

            the_build_list = devServ.CreateBuildList(the_build_list);

            try
            {
                var do_adding_sn = devServ.AddDevicesToBuildListFromRange(the_build_list.Id.Value, req.serial_number, req.serial_number);

                if (do_adding_sn != null)
                {
                    is_sukses = 1;

                    aa = devServ.PerformBuildListAction(the_build_list.Id.Value);

                    var bb = devServ.GetAcceptedList(aa.Id.Value, 0);

                    var cc = devServ.GetFailedList(aa.Id.Value, 0);
                }
