...
namespace web_api_icc_valsys_no_mvc.Controllers
{
    public class ShippingOrderController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/shippingorder/LinkDeviceThenSipped_2")]
        public HttpResponseMessage LinkDeviceThenSipped_2(NewLinkParams the_params)
        {
            List<DeviceMappings> serialNumberCollection = new List<DeviceMappings>();

            foreach(var sn in the_params.the_serial_number_list)
            {
                serialNumberCollection.Add(new DeviceMappings { SerialNumber = sn });
            }

            #region AuthorizationHeader
            //Here we assign our login credentials.
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(the_params.username_ad, the_params.password_ad);
            ...
            var get_cust = customersService.GetCustomer(the_params.cust_id);

            try
            {
                ProcessShippingOrder(get_cust, serialNumberCollection, the_params);
        ...
        private void ProcessShippingOrder(Customer customer, List<DeviceMappings> serialNumberCollection, NewLinkParams the_parameter)
        {
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(the_parameter.username_ad, the_parameter.password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);

            var OrderManagementService = AsmRepository.AllServices.GetOrderManagementService(ah);

            var shippingOrder = OrderManagementService.GetShippedOrders(customer.Id.Value, 0).Items.Find(t => t.StatusId == Convert.ToInt32(ShippingOrderStatus.Pending));

            if(shippingOrder != null)
            {
                ChangeShippingOrderStatusToMayShip(customer.Id.Value, the_parameter);
                AttachSerialNumberToDeviceForShipping(customer, serialNumberCollection, the_parameter);
            }
            else
                AttachSerialNumberToDeviceForShipping(customer, serialNumberCollection, the_parameter);
        ...
        private static void ChangeShippingOrderStatusToMayShip(int customer, NewLinkParams the_parameters)
        {
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(the_parameters.username_ad, the_parameters.password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);
            ...
            try
            {
                ...
                var shippingOrder = OrderManagementService.GetShippedOrders(customer, 0).Items.Find(t => t.StatusId == Convert.ToInt32(ShippingOrderStatus.Pending));
                ...
                OrderManagementService.UpdateShippingOrderStatus(shippingOrder.Id.Value, Convert.ToInt32(ShippingOrderReason.MayShip));
            }
            catch (Exception ex)
            {
        ...
        private static void AttachSerialNumberToDeviceForShipping(Customer customer, List<DeviceMappings> serialNumberCollection, NewLinkParams the_parameter)
        {
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(the_parameter.username_ad, the_parameter.password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);

            var deviceService = AsmRepository.AllServices.GetDevicesService(ah);
            var productCatalogConfigurationService = AsmRepository.AllServices.GetProductCatalogConfigurationService(ah);
            var viewFacadeService = AsmRepository.AllServices.GetViewFacadeService(ah);
            var orderManagementService = AsmRepository.AllServices.GetOrderManagementService(ah);
            

            //Get all device that valid for shipping
            foreach (var serialNumberItem in serialNumberCollection)
            {
                var device = deviceService.GetDeviceBySerialNumber(serialNumberItem.SerialNumber);

                if (device != null)
                {
                    var hardwareModel = productCatalogConfigurationService.GetHardwareModel(device.ModelId.Value);
                    if (device.StatusId.Value == Convert.ToInt32(DeviceStatus.InStock) ||
                        device.StatusId.Value == Convert.ToInt32(DeviceStatus.RefurbishedStock) ||
                        device.StatusId.Value == Convert.ToInt32(DeviceStatus.RepairedStock))
                    {
                        serialNumberItem.ModelId = hardwareModel.Id.Value;
                        serialNumberItem.TechnicalProductId = hardwareModel.TechnicalProductId.Value;
                        serialNumberItem.IsValidForShipping = true;

                        Console.WriteLine(hardwareModel.Description + " with serial number " + serialNumberItem.SerialNumber + " is allowed to attached to device.");
                    }
                    else
                    {
                        serialNumberItem.IsValidForShipping = false;

                        Console.WriteLine(hardwareModel.Description + " with serial number " + serialNumberItem.SerialNumber + " is not in allowed capture status!");
                    }
                }
                else
                    Console.WriteLine("Serial number " + serialNumberItem.SerialNumber + " is not valid serial number!");

            }

            // Get the hardware agreement detail of customer
            var agreementDetailViewCollection = viewFacadeService.GetAgreementDetailView(new BaseQueryRequest()
            {
                FilterCriteria = Op.Eq("DeviceIncluded", true) & Op.Eq("CustomerId", customer.Id) & Op.Gt("Id", 0) & Op.IsNull("ProvisionedDevices"),
            ...
            AgreementDetailView agreementDetailView = new AgreementDetailView();
            if (agreementDetailViewCollection.TotalCount == 0)
                Console.WriteLine("Hardware product is not captured, can't ship the shipping order for customer id : " + customer.Id);
            else
                agreementDetailView = agreementDetailViewCollection.FirstOrDefault();

            var shippingOrderId = GetShippingOrderByAgreementDetailId(agreementDetailView.Id.Value, the_parameter.username_ad, the_parameter.password_ad);
            Console.WriteLine("Found Shipping Order Id : " + shippingOrderId);

            // get the May shipping order and shipping order line( with smartcard or Sim card in it).
            var shippingOrderItem = orderManagementService.GetShippedOrders(customer.Id.Value, 0).Items.Find(t => (t.StatusId == Convert.ToInt32(ShippingOrderStatus.MayShip) && t.Id.Value == shippingOrderId));

            if (shippingOrderItem == null)
            {
                Console.WriteLine("No Shipping Order with status May Ship, please check customer id : " + customer.Id);
            }

            foreach (var shippingOrderLineItem in shippingOrderItem.ShippingOrderLines)
            {
                var serialNumberToShip = serialNumberCollection.Find(t => t.TechnicalProductId == shippingOrderLineItem.TechnicalProductId && t.IsValidForShipping == true);
                if (serialNumberToShip != null)
                    CreateBuildListForShipping(serialNumberToShip, shippingOrderLineItem, shippingOrderItem, the_parameter.username_ad, the_parameter.password_ad);

                serialNumberCollection.Remove(serialNumberToShip);
            }

            try
            {
                orderManagementService.ShipOrder(shippingOrderItem, Convert.ToInt32(ShippingOrderReason.Ship), null);
            }
            catch (Exception ex)
            {
                Console.WriteLine("Errors : " + ex.Message);
        ...
        private static int GetShippingOrderByAgreementDetailId(int agreementDetailId, string username_ad, string password_ad)
        {
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(username_ad, password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);
            ...
            try
            {
                var shippingOrderLineCollection = orderManagementService.FindShippingOrderLines(new BaseQueryRequest()
                {
                    FilterCriteria = Op.Eq("AgreementDetailId", agreementDetailId),
                    PageCriteria = new PageCriteria() { Page = 0, PageSize = 100 }
                });

                if (shippingOrderLineCollection.Items.Count > 0)
                    return shippingOrderLineCollection.FirstOrDefault().ShippingOrderId.Value;
                else
                    return 0;
            }
            catch (Exception ex)
        ...
        private static void CreateBuildListForShipping(DeviceMappings serialNumberItem, ShippingOrderLine shippingOrderLine, ShippingOrder shippingOrder, string username_ad, string password_ad)
        {
            Authentication_class var_auth = new Authentication_class();
            AuthenticationHeader ah = var_auth.getAuthHeader(username_ad, password_ad);
            AsmRepository.SetServiceLocationUrl(var_auth.var_service_location_url);
            ...
            try
            {
                var buildListParameter = new BuildList
                {
                    OrderLineId = shippingOrderLine.Id,
                    OrderId = shippingOrder.Id,
                    ModelId = serialNumberItem.ModelId,
                    StockHandlerId = shippingOrder.ShipFromStockHandlerId,
                    TransactionType = BuildListTransactionType.ShippingSerializedProducts
                };

                var buildList = deviceService.CreateBuildList(buildListParameter);

                var deviceBuildList = deviceService.AddDeviceToBuildListManually(buildList.Id.Value, serialNumberItem.SerialNumber);

                if (deviceBuildList.Accepted.Value)
                    deviceService.PerformBuildListAction(buildList.Id.Value);
                else
                    Console.WriteLine("Failed to attach decoder with SN : " + serialNumberItem.SerialNumber + " to Shipping Order : Error : " + deviceBuildList.Error);
